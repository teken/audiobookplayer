							...
							<div style={{
							...
							}}>
								{
									orderAuthors.map(author => {
										return this.props.savedTimeWorks.filter(x => x.author === author).map(book => this.props.savedBook(this.renderTile, book))
									})
								}
							</div>
						</div>
							...
							<div style={{
								...
								{orderAuthors.map(author => {
									const items = this.props.libraryWorks
										...
										.map(book => this.props.libraryBook(this.renderTile, book))
										.filter(x => x)
										.reduce((acc, val, i) => i%(this.gridWidthCellCount-1) === 0 ?acc.concat(<span key={`spacer-start-${author}-${i}`}/>,val) : acc.concat(val), [])
										.slice(1);

									return [
										items.length > 0 && <h2 key={`title-${author}`} style={{textAlign:'left'}}>{author}</h2>,
										...items,
										items.length > 0 && [...new Array(this.gridWidthCellCount-((items.length+1)%this.gridWidthCellCount)).keys()].map(x => <span key={`spacer-end-${author}-${x}`}/>)
									]})
								}
							</div>
						</div>
